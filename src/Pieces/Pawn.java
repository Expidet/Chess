...
import Promotion.PromotionFrame;
...
public class Pawn extends Piece{
    ...
    public Pawn(Teams team, Field startField, int pieceId) {
        super("chessPieces/" + team.name().toLowerCase() + "_pawn.svg", team, startField, pieceId);
    }
    ...
    @Override
    public void moved() {
        if((this.getCurrentField().getRow() == 7 && this.getTeam() == Teams.BLACK) || (this.getCurrentField().getRow() == 0 && this.getTeam() == Teams.WHITE)){
            GameManager.setChoosingPiece(true);
            PromotionFrame promotionFrame = new PromotionFrame(this);
            promotionFrame.pack();
